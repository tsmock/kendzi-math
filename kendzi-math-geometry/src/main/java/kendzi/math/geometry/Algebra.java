...
public class Algebra {
    ...
    @Deprecated public static Double projectPointPerpendicularToLine(Double lineA, Double lineB, Double point) {

        double sqr1 = lineA.x - lineB.x;
        double sqr2 = lineA.y - lineB.y;
        double u =
                ((point.x - lineA.x) * (lineB.x - lineA.x) + (point.y - lineA.y) * (lineB.y - lineA.y)) / (sqr1 * sqr1
                        + sqr2 * sqr2);

        return new Double((lineA.x + (lineB.x - lineA.x) * u), (lineA.y + (lineB.y - lineA.y) * u));
    }

    @Deprecated public static Point2d projectPointPerpendicularToLine(Point2d lineA, Point2d lineB, Point2d point) {
        ...
        return new Point2d((lineA.x + (lineB.x - lineA.x) * u), (lineA.y + (lineB.y - lineA.y) * u));
    }
    ...
    public static Vector3d orthogonalProjection(Vector3d unitVector, Vector3d vectorToProject) {
        ...
        return new Vector3d(px * ax * ax + py * ax * ay + pz * ax * az, px * ax * ay + py * ay * ay + pz * ay * az,
